...
public class NoteSpawner : MonoBehaviour
{
   ...
   [SerializeField] private MIDIManager midiManager;
    ...
    void Start()
    {
        blNotePool = new ObjectPool<NoteBehavior>(() =>
        {
            ...
            note.InIt(KillNote, midiManager.NoteOn , midiManager.NoteOff);
            return note;
        }
    ...
    private void Update()
    {
        if (canSpawn)
        {
            ...
            if (Time.time - startTime > totalMIDITime + 5.0f)
    ...
    private void LoadDataFromMidiFile()
    {
        ...
        MidiFile midiFile = midiManager.GetMIDIFromFile(midiFilePath);
        tempoMap = midiFile.GetTempoMap();

        totalMIDITime =(float) midiFile.GetTimedEvents().Last().TimeAs<MetricTimeSpan>(tempoMap).TotalSeconds;
